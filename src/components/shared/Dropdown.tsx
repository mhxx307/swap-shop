...
const Dropdown = ({ dataList, className }: DropdownProps) => {
    ...
    return (
        <div className={classNames('w-full font-medium', className)}>
            <div
            ...
            >
                ...
                {dataList?.map((item) => (
                    <Link
                        key={item?.label}
                        href={item?.path}
                        className={`py-4 pl-[20px] text-xl font-normal hover:text-black capitalize transition-colors border-b-[1px] border-b-[#eee]
            ${
